...
/// @brief Bridge between ESPHome native RGB light entities and Nixie hardware
/// channels.
...
/// @brief Built-in light effect that delegates animation modes to
/// HackPackNixieClock.
class HackPackNixieEffect : public light::LightEffect {
public:
  HackPackNixieEffect(const std::string &name, ColorMode mode,
                      HackPackNixieClock *parent)
      : light::LightEffect(nullptr), name_holder_(name), mode_(mode),
        parent_(parent) {
    this->name_ = this->name_holder_.c_str();
  }
  HackPackNixieEffect(const char *name, ColorMode mode,
                      HackPackNixieClock *parent)
...
/// @brief Listener that synchronizes linked brightness between Panel and
/// Underglow channels.
class HackPackNixieLightListener : public light::LightRemoteValuesListener {
public:
  HackPackNixieLightListener(HackPackNixieClock *parent, LightType type)
