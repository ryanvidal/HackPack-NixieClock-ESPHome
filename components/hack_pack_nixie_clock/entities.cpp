...
void HackPackNixieSwitch::setup() {
  ...
  case SwitchType::FORMAT_24HR:
    initial_state = parent_->get_24hr_mode();
    break;
  case SwitchType::LEADING_ZERO:
    initial_state = parent_->get_leading_zero();
    break;
  case SwitchType::COLON_BLINKING:
    initial_state = parent_->get_colon_blinking();
    break;
  case SwitchType::AM_PM_INDICATORS:
    initial_state = parent_->get_am_pm_indicators();
    break;
  case SwitchType::PERIODIC_FACES:
    initial_state = parent_->get_face_animations();
    break;
  case SwitchType::PHYSICAL_BUTTONS:
    initial_state = parent_->get_physical_buttons();
    break;
  case SwitchType::LINK_BRIGHTNESS:
    initial_state = parent_->get_link_brightness();
    break;
  case SwitchType::ALARM_ENABLED:
    initial_state = parent_->is_alarm_armed();
    break;
  case SwitchType::RECORD_SOUND:
    initial_state = false;
    break;
  case SwitchType::TIMER_RUNNING:
    initial_state = parent_->is_timer_running();
    break;
  }
...
void HackPackNixieSwitch::write_state(bool state) {
  ...
  switch (type_) {
  case SwitchType::FORMAT_24HR:
    parent_->set_24hr_mode(state);
    break;
  case SwitchType::LEADING_ZERO:
    parent_->set_leading_zero(state);
    break;
  case SwitchType::COLON_BLINKING:
    parent_->set_colon_blinking(state);
    break;
  case SwitchType::AM_PM_INDICATORS:
    parent_->set_am_pm_indicators(state);
    break;
  case SwitchType::PERIODIC_FACES:
    parent_->set_face_animations(state);
    break;
  case SwitchType::PHYSICAL_BUTTONS:
    parent_->set_physical_buttons(state);
    break;
  case SwitchType::LINK_BRIGHTNESS:
    parent_->set_link_brightness(state);
    break;
  case SwitchType::ALARM_ENABLED:
    if (state)
      parent_->arm_alarm();
    else
      parent_->disarm_alarm();
    break;
  case SwitchType::RECORD_SOUND:
    parent_->set_record_sound(state);
    break;
  case SwitchType::TIMER_RUNNING:
    if (state)
      parent_->start_timer();
...
void HackPackNixieButton::press_action() {
  if (!parent_)
    return;
  switch (type_) {
  case ButtonType::RESET_TIMER:
    parent_->reset_timer();
    break;
  case ButtonType::STOP_ALARM:
    parent_->stop_alarm();
    break;
  case ButtonType::TRIGGER_FACE:
    parent_->trigger_face_animation();
    break;
  case ButtonType::TRIGGER_SLOT_MACHINE:
    parent_->trigger_slot_machine(2500);
    break;
  case ButtonType::PLAY_SOUND:
    parent_->play_beep(150);
    break;
  }
}
#endif
...
void HackPackNixieSelect::setup() {
  ...
  case SelectType::COLON_COLOR_MODE: {
    ...
    case COLON_AUTO_BLEND:
      publish_state("Auto Blend");
      break;
    case COLON_MATCH_UNDERGLOW:
      publish_state("Match Underglow");
      break;
    case COLON_FIXED:
    default:
      publish_state("Fixed");
...
void HackPackNixieSelect::control(const std::string &value) {
  ...
  case SelectType::COLON_COLOR_MODE:
    if (value == "Auto Blend")
      parent_->set_colon_mode(COLON_AUTO_BLEND);
    else if (value == "Match Underglow")
      parent_->set_colon_mode(COLON_MATCH_UNDERGLOW);
    else if (value == "Fixed")
      parent_->set_colon_mode(COLON_FIXED);
    break;
  }
  publish_state(value);
}
